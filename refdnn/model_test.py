...
class REFDNN:
    ...
    def fit(self,
        ...
        for step in range(1, self.training_steps+1):
            ## 1) training
            _, loss_train = self.sess.run([self.TRAIN_adam, self.LOSS],
    ...
    def _create_graph(self):
        ...
        ## LOSS
        self.LOSS = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=O_DNN, labels=tf.cast(Y_batch, dtype=tf.float32)))
