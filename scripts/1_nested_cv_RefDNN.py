...
def main():
    ...
    for k, (idx_train, idx_test) in enumerate(kf.split(X=np.zeros(len(dataset)), y=dataset.get_drugs())):
        ...
        if verbose > 0:
            ...
            print('[OUTER] [{}/{}] BEST_TEST_Precision : {:.3f}'.format(k+1, kf.get_n_splits(), Precision_outer_k))
            print('[OUTER] [{}/{}] BEST_TEST_Recall : {:.3f}'.format(k+1, kf.get_n_splits(), Recall_outer_k))
            print('[OUTER] [{}/{}] BEST_TEST_F1 : {:.3f}'.format(k+1, kf.get_n_splits(), F1_outer_k))
...
def fitness(hyperparameters):
    ...
    if verbose > 1:
        print('[BAYESIAN SEARCH] [{}/{}] NOW EVALUATING PARAMETERS IN THE BAYESIAN SEARCH LOOP...'.format(fitness_step, numbayesiansearch))
    ...
    for k, (idx_construction, idx_validation) in enumerate(kf.split(X=np.zeros_like(fitness_idx_train), y=dataset.get_drugs()[fitness_idx_train])):
        ...
        if verbose > 1:
            print('[INNER] [{}/{}] NOW FITTING A MODEL IN THE INNER LOOP...'.format(k+1, kf.get_n_splits()))
    ...
    if verbose > 1:
        print('[BAYESIAN SEARCH] [{}/{}] HIDDEN_UNITS: {}'.format(fitness_step, numbayesiansearch, HIDDEN_UNITS))
        print('[BAYESIAN SEARCH] [{}/{}] LEARNING_RATE_FTRL: {:.3e}'.format(fitness_step, numbayesiansearch, LEARNING_RATE_FTRL))
        print('[BAYESIAN SEARCH] [{}/{}] LEARNING_RATE_ADAM: {:.3e}'.format(fitness_step, numbayesiansearch, LEARNING_RATE_ADAM))
        print('[BAYESIAN SEARCH] [{}/{}] L1_REGULARIZATION_STRENGTH: {:.3e}'.format(fitness_step, numbayesiansearch, L1_REGULARIZATION_STRENGTH))
        print('[BAYESIAN SEARCH] [{}/{}] L2_REGULARIZATION_STRENGTH: {:.3e}'.format(fitness_step, numbayesiansearch, L2_REGULARIZATION_STRENGTH))
        print('[BAYESIAN SEARCH] [{}/{}] TRAINING_ACCURACY: {:.3f}'.format(fitness_step, numbayesiansearch, training_accuracy))
