...
def main():
    ...
    for k, (idx_train, idx_test) in enumerate(kf.split(X=np.zeros(len(dataset)), groups=dataset.get_drugs())):
        ...
        BEST_ALPHA = 0.010196888209425372
        BEST_L1_RATIO = 0.43973347551563247
        BEST_ETA0 = 0.02181201561605224
    ...
    res = pd.DataFrame.from_dict({'DRUGNAME':DRUG_outer,
                                  ...
                                  'L1_RATIO':BEST_L1_RATIO,
                                  'ETA0':BEST_ETA0})
    #res = res[['DRUGNAME', 'ACCURACY', 'AUCROC', 'AUCPR', 'ALPHA', 'L1_RATIO', 'ETA0']]
